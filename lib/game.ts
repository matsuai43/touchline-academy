...
function simulateSegment(s:State){const m=s.match!,team=roster(s),rating=strength(s),style=m.fixture.style;const advantage=m.tactic==='possession'&&style==='counter'||m.tactic==='counter'&&style==='press'||m.tactic==='press'&&style==='possession'?1.17:m.tactic!=='balanced'&&m.tactic!==style?.87:1;
 ...
 let goal=false;const events:[number,string][]=[];for(let i=0;i<3;i++){const minute=m.minute+Math.round(1+rand(s)*14);for(let side=0;side<2;side++){const rate=side===0?homeRate:awayRate;if(rand(s)<rate*1.9){m.shots[side]++;const chance=clamp(.15+rand(s)*.22+(side===0?(stat('shoot')-55)/500:0),.1,.5);m.xg[side]+=chance;if(rand(s)<chance){if(side===0){m.home++;const scorers=team.filter(p=>p.pos==='FW'||p.pos==='MF'),scorer=pick(s,scorers.length?scorers:team);scorer.goals++;events.push([minute,`${minute}′ GOAL！ ${scorer.name}がネットを揺らす！`]);}else{m.away++;events.push([minute,`${minute}′ 失点。${m.fixture.opponent}がゴール。`]);}m.lastSide=side;goal=true;}}}}
 events.sort((x,y)=>x[0]-y[0]);for(const e of events)m.logs.unshift(e[1]);
